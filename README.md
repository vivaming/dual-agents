...
When using the CLI orchestration path, use `dual-agents start-unit` to create/update `.dual-agents/run-state.json`. By default it starts in implementation mode; pass `--start-mode review` for an explicit pre-implementation gate, or rely on `--start-mode auto` with `--task-summary "..."` and optionally `--task-file /path/to/task.md` to infer review mode from review/spec/design language. In auto mode the CLI also tries a best-effort `epic/**/*.md` match from the unit slug. Then use `dual-agents review-gate` to run Codex and persist the review artifact plus updated bounded-unit state.
If you already have a saved review artifact and do not want to run Codex again, use `dual-agents submit-review-artifact --unit-slug <unit> --mode <lead|final> --review-file <path> --repo-root <repo>` to validate and advance workflow state from that existing file.
